# Доработал под Python 3.5 в части 
# где используется конструкция f:"{self.name}_.gif"
import math
import os
import random
import time
	...
	def get_pic_name(self):
		for missile in our_missiles:
			if missile.distance(self.x, self.y) < 50:
				pic_name = self.name + "_opened.gif"
				break
		else:
			pic_name = self.name +".gif"
		return pic_name
	...
	for enemy_missile in enemy_missiles:
		...
		for building in buildings:
			if enemy_missile.distance(building.x, building.y) < enemy_missile.radius * 10:
				building.health -= 100
				# print(building.name, building.health)
